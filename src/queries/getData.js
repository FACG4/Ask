...
const getUserData=(user_id,cb)=>{
    dbConnections.query(`select questions.body as questionBody ,answers.body  as answerBody from questions left join answers on questions.id=answers.question_id where questions.user_id=${user_id}`,(err,res)=>{
        if(err) cb(err);
        else{
            cb(null,res.rows);
        }
